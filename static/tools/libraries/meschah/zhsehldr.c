...
ZVEC *zhhtrvec(hh, beta, i0, in, out)
    ...
    out = zv_copy(in, out);
    __zmltadd__(&(out->ve[i0]), &(hh->ve[i0]), scale, (int) (in->dim - i0), Z_NOCONJ);
    /************************************************************
     for ( i=i0; i<in->dim; i++ )
...
ZMAT *zhhtrrows(M, i0, j0, hh, beta)
    ...
    for (i = i0; i < M->m; i++) { /* compute inner product */
        ...
        /* do operation */
        __zmltadd__(&(M->me[i][j0]), &(hh->ve[j0]), scale, (int) (M->n - j0), Z_CONJ);
        /**************************************************
         for ( j = j0; j < M->n; j++ )
...
ZMAT *_zhhtrcols(M, i0, j0, hh, beta, w)
    ZMAT *M;int i0, j0;ZVEC *hh;double beta;ZVEC *w; {
    /* Real	ip, scale; */
    complex scale;
    int i /*, k */;
    ...
    for (i = i0; i < M->m; i++)
        /* if ( hh->ve[i] != 0.0 ) */
        if (!is_zero(hh->ve[i]))
            __zmltadd__(&(w->ve[j0]), &(M->me[i][j0]), hh->ve[i], (int) (M->n - j0), Z_CONJ);
    for (i = i0; i < M->m; i++)
        /* if ( hh->ve[i] != 0.0 ) */
        if (!is_zero(hh->ve[i])) {
            scale.re = -beta * hh->ve[i].re;
            scale.im = -beta * hh->ve[i].im;
            __zmltadd__(&(M->me[i][j0]), &(w->ve[j0]), scale, (int) (M->n - j0), Z_CONJ);
        }

...
#endif
{
    ...
    if (i1 < 0 || j1 < 0 || i2 < 0 || j2 < 0 || i1 >= A->m || j1 >= A->n || i2 >= A->m || j2 >= A->n) {
...
#endif
{
    ...
    while (row2 >= 0 && row2 < i2) {
        ...
        if (!done_list->ive[row2]) {
            ...
            tracecatch(bkp_swap_elt(A, row2, i2, idx2, i1, row2, tmp_idx1), "bkp_interchange");
        }
    ...
    while (idx1 < r1->len) {
        ...
        if (!done_list->ive[e1->col]) {
            ...
            tracecatch(bkp_swap_elt(A, i1, e1->col, idx1, e1->col, i2, tmp_idx2), "bkp_interchange");
    ...
    while (idx1 < r1->len) {
        ...
        if (!done_list->ive[e1->col]) {
            ...
            tracecatch(bkp_swap_elt(A, i1, e1->col, idx1, i2, e1->col, tmp_idx2), "bkp_interchange");
            done_list->ive[e1->col] = TRUE;
        }
        idx1++;
        e1++;
    ...
    while (idx2 < r2->len) {
        ...
        if (!done_list->ive[e2->col]) {
            ...
            tracecatch(bkp_swap_elt(A, i2, e2->col, idx2, i1, e2->col, tmp_idx1), "bkp_interchange");
...
#endif
{
    ...
    for (i = 0; i < n - 1; i = onebyone ? i + 1 : i + 2) {
        ...
        for (deg = 0; deg <= n; deg++) {
            ...
            while (list_idx < deg_list->dim && deg_list->ive[list_idx] <= deg) {
                ...
                for (list_idx2 = 0; list_idx2 < list_idx; list_idx2++) {
                    ...
                    ajl = (j > l) ? fabs(unord_get_val(A, l, j)) : fabs(unord_get_val(A, j, l));
                    det = fabs(ajj * all - ajl * ajl);
                    if (det == 0.0)
                        continue;
                    max_j = max_row_col(A, i, j, l);
                    max_l = max_row_col(A, i, l, j);
                    if (tol * (all * max_j + ajl * max_l) < det && tol * (ajl * max_j + ajj * max_l) < det) {
        ...
        if (best_j >= 0 && onebyone) {
            ...
            /* can't pivot &/or nothing to pivot */
            continue;
                ...
                for (s_idx2 = s_idx; s_idx2 < scan_row->dim; s_idx2++) {
                    ...
                    if (scan_row->ive[s_idx2] == j) { /* no fill-in -- can be done directly */
                        ...
                        if (row->len >= row->maxlen) {
                            tracecatch(sprow_xpd(row,2*row->maxlen+1,TYPE_SPMAT), "spBKPfactor");
                        }
        ...
        { /* do 2 x 2 pivot */
            ...
            for (s_idx = 0; s_idx < scan_row->dim; s_idx++) {
                ...
                while (s_idx2 < scan_row->dim) {
                    ...
                    e_ip1k =
                            (idx1_k < 0) ? (row_elt *) NULL : &(r1_piv->elt[idx1_k]);
                ...
                if (idx >= 0) {
                    ...
                    if (i > 0) {
                        tracecatch(chase_col(A, j, &old_row, &old_idx, i - 1), "spBKPfactor");
                    }
                    /* sprow_set_val(r_piv,j,s); */
                    idx = r_piv->len;
                    if (r_piv->len >= r_piv->maxlen) {
                        tracecatch(sprow_xpd(r_piv,2*r_piv->maxlen+1, TYPE_SPMAT), "spBKPfactor");
                    }
                ...
                if (idx1 >= 0) {
                    ...
                    if (r1_piv->len >= r1_piv->maxlen) {
                        tracecatch(sprow_xpd(r1_piv,2*r1_piv->maxlen+1, TYPE_SPMAT), "spBKPfactor");
                    }

...
SPMAT *sp_get(int, int, int), *sp_copy(const SPMAT *),
        *sp_copy2(const SPMAT *, SPMAT *), *sp_zero(SPMAT *),
        *sp_resize(SPMAT *, int, int), *sp_compact(SPMAT *, double);
double sp_get_val(const SPMAT *, int, int),
        sp_set_val(SPMAT *, int, int, double);
...
SPROW
        *sprow_add(const SPROW *r1, const SPROW *r2, int j0, SPROW *r_out, int type),

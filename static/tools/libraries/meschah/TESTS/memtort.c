...
int foo_1_free(f)
    ...
    if (f != NULL) {
        if (mem_info_is_on()) {
            mem_bytes_list(TYPE_FOO_1, sizeof(FOO_1) + f->dim * sizeof(Real[10]), 0, FOO_LIST);
            mem_numvar_list(TYPE_FOO_1, -1, FOO_LIST);
        }
...
int foo_2_free(f)
    ...
    if (f != NULL) {
        if (mem_info_is_on()) {
            mem_bytes_list(TYPE_FOO_2, sizeof(FOO_2) + f->dim * sizeof(Real[2]), 0, FOO_LIST);
            mem_numvar_list(TYPE_FOO_2, -1, FOO_LIST);
        }
...
int (*foo_free_func[FOO_NUM_TYPES])() = {
...
#ifdef ANSI_C
        mem_stat_reg_vars(0, TYPE_VEC, __FILE__, __LINE__, &xt1, &xt2, &xt3, &xt4, &yt1, &yt2, &yt3, &yt4, NULL);
#else
#ifdef VARARGS
    ...
    void main(argc, argv)
        ...
        mem_attach_list(FOO_LIST, FOO_NUM_TYPES, foo_type_name, foo_free_func, foo_info_sum);

...
static char *test_err_list[] = { "unknown error", /* 0 */
...
int main(argc, argv)
    ...
    catch(E_NULL, catchall(v_add(VNULL,VNULL,VNULL); errmesg("tracecatch() failure"), printf("# tracecatch() caught error\n"); error(E_NULL,"main")); errmesg("catch() failure"), printf("# catch() caught E_NULL error\n"));
    ...
    s2 = v_norm2(x);
    s3 = v_norm_inf(x);
    if (fabs(s1 - x->dim) >= MACHEPS * x->dim || fabs(s2 - sqrt((Real) (x->dim))) >= MACHEPS * x->dim || fabs(s3 - 1.0) >= MACHEPS)

...
#endif
{
    ...
    while (k_min < n) {
        ...
        while (!split) {
            ...
            for (k = k_min; k < k_max; k++)
                if (fabs(A_me[k + 1][k]) < MACHEPS * (fabs(A_me[k][k]) + fabs(A_me[k + 1][k + 1]))) {
                    A_me[k + 1][k] = 0.0;
                    split = TRUE;
                }
        }
    ...
    for (i = 0; i < A->m; i++)
        for (j = 0; j < i - 1; j++)
            A_me[i][j] = 0.0;
    for (i = 0; i < A->m - 1; i++)
        if (fabs(A_me[i + 1][i]) < MACHEPS * (fabs(A_me[i][i]) + fabs(A_me[i + 1][i + 1])))
            A_me[i + 1][i] = 0.0;
...
#endif
{
    ...
    Real l_re, l_im, det_re, det_im, invdet_re, invdet_im, val1_re, val1_im,
            val2_re, val2_im, tmp_val1_re, tmp_val1_im, tmp_val2_re,
            tmp_val2_im, **T_me;
    ...
    if (T->m != T->n || X_re->m != X_re->n || (Q != MNULL && Q->m != Q->n) || (X_im != MNULL && X_im->m != X_im->n))
        error(E_SQUARE, "schur_vecs");
    if (T->m != X_re->m || (Q != MNULL && T->m != Q->m) || (X_im != MNULL && T->m != X_im->m))

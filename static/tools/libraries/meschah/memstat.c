...
#endif
{
    ...
    if (mem_stat_var[k].var == var) {
        ...
        j = k;
        while (mem_stat_var[j].var != var && j < MEM_HASHSIZE && mem_stat_var[j].var != NULL)
            j++;
...
#endif
{
    ...
    if (type < 0 || type >= mem_connect[list].ntypes || mem_connect[list].free_funcs[type] == NULL) {
...
#endif
{
    ...
    if (list < 0 || list >= MEM_CONNECT_MAX_LISTS || mem_connect[list].free_funcs == NULL)
        return -1;
    ...
    for (i = 0; i < mem_hash_idx_end; i++) {
        ...
        if (j == 0)
            ...
            fprintf(fp, " %d.  var = 0x%p, type = %s, mark = %d\n", k, mem_stat_var[j].var,
                    mem_stat_var[j].type < mem_connect[list].ntypes && mem_connect[list].free_funcs[mem_stat_var[j].type] != NULL ? mem_connect[list].type_names[(int) mem_stat_var[j].type] : "???", mem_stat_var[j].mark);
            k++;
        }
    }

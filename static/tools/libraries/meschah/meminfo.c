...
static int (*mem_free_funcs[MEM_NUM_STD_TYPES])() = {
    ...
    MEM_CONNECT mem_connect[MEM_CONNECT_MAX_LISTS] = { { mem_type_names, mem_free_funcs, MEM_NUM_STD_TYPES, mem_info_sum } };
...
#endif
    {
        ...
        if (mem_connect[list].type_names != NULL && mem_connect[list].free_funcs != NULL && mem_connect[list].info_sum != NULL)
            return TRUE;
        else
            return FALSE;
    }
...
#endif
    {
        if (list < 0 || list >= MEM_CONNECT_MAX_LISTS)
            return 0l;
        if (!mem_switched_on || type < 0 || type >= mem_connect[list].ntypes || mem_connect[list].free_funcs[type] == NULL)
            return 0l;

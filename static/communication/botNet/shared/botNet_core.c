 ...
 */
int bn_init(){
#if MYADDRX!=0 || MYADDRU!=0 || MYADDRT!=0
    int ret=0;
#endif
...
int bn_genericSend(sMsg *msg){
    // sets source address
    msg->header.srcAddr = ((MYADDRX?:MYADDRI)?:MYADDRU)?:MYADDRT;
...
#if MYADDRT!=0
    if ( (ret=TCP_receive(&temp.msg)) > 0 ) {
        bn_pushInBufLast(&temp.msg,IF_TCP);
    }
    else if (ret<0) return ret;
    count+=ret;
#endif
...
#ifdef DEBUG_PC_RT
    {
        ...
        if (pTmp->msg.header.type == E_TRACEROUTE_REQUEST){
            ...
            if (
                ...
                pTmp->msg.header.destAddr == MYADDRT
            ){
                bn_freeInBufFirst();
                return count;
            }
...
#if MYADDRT!=0
    if ((msg->header.destAddr & SUBNET_MASK) == (MYADDRT & SUBNET_MASK) ){
        if (ifFrom!=IF_TCP ) {
            routeInfo->ifTo=IF_TCP;
            routeInfo->nextHop=msg->header.destAddr;
            return;
        }
        else {
